...
public class DataManager
{
    ...
    public void CreateProject(string ProjectName, string RequestSource, string RequestType, string SkillsRequired, string RequestorName, string RequestorTeam, string EstinmatedEffort, string TargetedCompletionDate, string ProjectStatus)
    {
        using (DbConnection con = CreateConnection())
        {
            con.Open();

            var cmd = CreateCommand("insert into [Project] ([ProjectName],[RequestSource],[RequestType],[SkillsRequired],[RequestorName],[RequestorTeam],[EstinmatedEffort],[TargetedCompletionDate],[ProjectStatus]) values (@ProjectName, @RequestSource, @RequestType, @SkillsRequired, @RequestorName, @RequestorTeam, @EstinmatedEffort, @TargetedCompletionDate, @ProjectStatus)", con);
            AddParameterWithValue(cmd, "@ProjectName", ProjectName);
            AddParameterWithValue(cmd, "@RequestSource", RequestSource);
            AddParameterWithValue(cmd, "@RequestType", RequestType);
            AddParameterWithValue(cmd, "@SkillsRequired", SkillsRequired);
            AddParameterWithValue(cmd, "@RequestorName", RequestorName);
            AddParameterWithValue(cmd, "@RequestorTeam", RequestorTeam);
            AddParameterWithValue(cmd, "@EstinmatedEffort", EstinmatedEffort);
            AddParameterWithValue(cmd, "@TargetedCompletionDate", TargetedCompletionDate);
            AddParameterWithValue(cmd, "@ProjectStatus", ProjectStatus);
    ...
    {
        var da = CreateDataAdapter("select * from [User] where [UserLogin] = @username and [UserPassword] = @password");
    ...
    {
        var da = CreateDataAdapter("select * from [User] where [UserLogin] = @name");
        AddParameterWithValue(da.SelectCommand, "name", name);
        DataTable dt = new DataTable();
        da.Fill(dt);
        if (dt.Rows.Count == 1)
        {
    ...
    public string GetUserConfig(string userName, string key)
    {
        DataRow userId = GetUserByName(userName);
        var da = CreateDataAdapter("select * from [UserConfig] where [UserId] = @id and [UserConfigKey] = @key order by [UserConfigId]");
        AddParameterWithValue(da.SelectCommand, "id", userId[0]);
